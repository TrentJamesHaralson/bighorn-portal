...
function normalizeRole(r) {
  ...
  return (up === "ADMIN" || up === "ADMINISTRATOR") ? "ADMINISTRATOR" : "EMPLOYEE";
}
...
async function getOrCreateAccountByName(name) {
  ...
  let res = await pool.query("SELECT id FROM accounts WHERE name = $1", [nameTrim]);
...
async function main() {
  ...
  console.log(`Import done Success: ${ok}, Failed: ${fail}`);
  process.exit(0);
}
